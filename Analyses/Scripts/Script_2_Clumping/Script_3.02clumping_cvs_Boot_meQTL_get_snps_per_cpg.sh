#!/bin/bash
#SBATCH --job-name=c_meqtl_c
#SBATCH --output=/binder/Linda/Processing/P2_Omics_PlacentaTissue/03_Code_Scripts/log_files/clumps_meqtl_cvs/2/ind_submission_%A-%a.logs
...
input_dir=/binder/Linda/Processing/P2_Omics_PlacentaTissue/02_Data/clumping/cvs-meqtl_filesforclump
...
cpg=$(awk -v cpg_id=$n 'NR==cpg_id {print $1}' ${input_dir}/unique_eCpGs_cvs_for_clump.txt)

echo "SNP P" > snp_lst_for_ld_clump_${cpg}.txt
awk -v cg=$cpg '$1 ~ cg {print $2 " " $9}' ${input_dir}/bme_meqtl_cvs_for_clump.txt >> snp_lst_for_ld_clump_${cpg}.txt
