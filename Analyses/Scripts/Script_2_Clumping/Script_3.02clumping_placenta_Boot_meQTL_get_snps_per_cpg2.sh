...
#SBATCH --output=/binder/Linda/Processing/P2_Omics_PlacentaTissue/03_Code_Scripts/log_files/clumps_meqtl_placenta/2.2/ind_submission_%A-%a.logs
...
#SBATCH --part=pe
#SBATCH --exclude=pe5,pe3,pe4,pe8
...
cpg=$(awk -v cpg_id=$n 'NR==cpg_id {print $1}' ${input_dir}/missingCpGs1run.txt)

echo "SNP P" > snp_lst_for_ld_clump_${cpg}.txt
awk -v cg=$cpg '$1 ~ cg {print $2 " " $9}' ${input_dir}/bme_meqtl_placenta_for_clump2.txt >> snp_lst_for_ld_clump_${cpg}.txt
